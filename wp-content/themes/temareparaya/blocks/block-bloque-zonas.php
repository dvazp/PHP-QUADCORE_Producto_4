...
if ( $resultado && isset( $resultado['ok'] ) && $resultado['ok'] === true ) : ?>
    ...
    <div class="contenedor-zonas" style="background: none; padding:20px; font-family:sans-serif; max-width:600px; margin:auto; box-shadow: none;">
        <h2 style="color:#1e3653; font-size:1.4rem; margin-bottom:20px;">Distribución de Servicios por Zonas (API en vivo)</h2>
        
        <div class="grid-zonas" style="display:flex; flex-direction:column; gap:15px;">
            <?php foreach ( $resultado['data'] as $item ) : ?>
                <div class="tarjeta-zona" style="background:#bccad1; padding:12px; border-radius:6px; border-left:5px solid #1e3653; box-shadow:0 4px 8px rgba(0, 0, 0, 0.15);">
                    
                    <div class="info-tarjeta" style="display:flex; justify-content:space-between; margin-bottom:6px; font-size:0.95rem;">
                        <span class="nombre-zona" style="font-weight:bold; color:#1e3653;"><?php echo esc_html( $item['zona'] ); ?></span>
                        <span class="servicios-zona" style="color:#1e3653;"><strong><?php echo esc_html( $item['total_servicios'] ); ?></strong> servicios / <strong><?php echo esc_html( $item['porcentaje'] ); ?></strong>%</span>
                    </div>
                    
                    <div class="barra-progreso-bg" style="background:#e0e0e0; border-radius:10px; height:16px; width:100%; overflow:hidden;">
                        <div class="barra-progreso-relleno" style="background:#1e3653; height:100%; width:<?php echo esc_attr( $item['porcentaje'] ); ?>%; display:flex; align-items:center; justify-content:flex-end;">
...
<?php else : ?>
    <div style="padding: 20px; background: #bccad1; color: #856404; border-radius: 6px;">
